...
`runs/<run_id>/config.json`'dan `TrainingConfig`'i okur, `mlx-lm-lora==2.1.0`
...
- `mlx_lm_lora.train.run(args, training_callback=...)` — model/dataset
  yükleme, eğitim modu dispatch'i (sft/dpo/orpo/cpo/grpo) ve adapter kaydını
  uçtan uca yapan tek fonksiyon. `main()` CLI sarmalayıcısı `training_callback`
  kabul etmediği için doğrudan `run()` çağrılıyor.
...
SUPPORTED_TRAIN_MODES = {"sft", "dpo", "orpo", "cpo", "grpo"}

# train_mode başına referans modele ihtiyaç duyan modlar (kütüphane kaynağına
# göre): dpo ve grpo. orpo/cpo referans model kullanmıyor.
_MODES_NEEDING_REFERENCE_MODEL = {"dpo", "grpo"}
